...
Problem is that in Node 12 Module Not Found error differs from Node 10. bagpipes relies on error message text. This can temporary be fixed in this file: node_modules/bagpipes/lib/fittingTypes/user.js file:
var split = err.message.split(path.sep);
should be
var split = err.message.split('\n')[0].split(path.sep);

see: https://github.com/swagger-api/swagger-node/issues/586
*/
...
module.exports = function createFitting(pipes, fittingDef) {
  ...
  for (var i = 0; i < pipes.config.userFittingsDirs.length; i++) {
    ...
    try {
      ...
      //var split = err.message.split(path.sep); PATCH!
      var split = err.message.split('\n')[0].split(path.sep);
